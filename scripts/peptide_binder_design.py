#!/usr/bin/env python3
"""
Script: peptide_binder_design.py
Description: Design peptide binders using BoltzGen with peptide-anything protocol

Original Use Case: examples/use_case_2_peptide_binder_design.py
Dependencies Removed: scripts.run_boltzgen (inlined), loguru (simplified)

Usage:
    python scripts/peptide_binder_design.py --input <config_file> --output <output_dir>

Example:
    python scripts/peptide_binder_design.py --input examples/data/beetletert.yaml --output results/peptide_design
"""
...
DEFAULT_CONFIG = {
    ...
    "num_designs": 10,
    "budget": 2,
    "alpha": None,  # Diversity parameter for peptides
    "master_port": 29500,
    "verbose": True
}
...
def run_peptide_binder_design(
    ...
    **kwargs
    ...
    """
    Main function for peptide binder design using BoltzGen.
    ...
    Example:
        >>> result = run_peptide_binder_design("examples/data/beetletert.yaml", "results/peptide_design")
    ...
    if output_file is None:
        output_file = f"results/peptide_binder_{input_file.stem}"
    output_path = Path(output_file)

    log_info("=" * 80)
    log_info("BoltzGen Peptide Binder Design")
    log_info("=" * 80)
    log_info(f"Protocol: {config_merged['protocol']} (specialized for peptides)")
    log_info(f"Features: Filters cysteines by default, optimized diversity parameters")
    ...
    if config_merged.get("alpha") is not None:
        log_info(f"Alpha (diversity): {config_merged['alpha']}")
    ...
    # Add alpha parameter if specified (peptide-specific diversity control)
    if config_merged.get("alpha") is not None:
        cmd.extend(["--alpha", str(config_merged["alpha"])])
    ...
    # Log command
    log_info("Running BoltzGen with peptide-anything protocol:")
    log_info(f"  {' '.join(cmd)}")
    ...
    try:
        ...
        log_info("-" * 80)
        if return_code == 0:
            log_success("Peptide binder design completed successfully!")
            ...
            log_info("  - Cysteine filtering enabled by default")
            log_info("  - Lower diversity parameters for peptide optimization")
            log_info("  - Suitable for linear and cyclic peptides")
        else:
        ...
        return {
            ...
            "metadata": {
                ...
                "protocol_features": ["cysteine_filtering", "peptide_optimized_diversity"]
            }
        }
...
def main():
    ...
    parser.add_argument('--alpha', type=float, help='Diversity vs quality tradeoff (0.0=quality, 1.0=diversity)')
